...
class WindowSplittingExecution:
    ...
    @classmethod
    def execute_with_scaler(cls, data_file: str, 
                     batch_split: bool, 
                     validation_split: bool, 
                     test_size: float, 
                     seed: int, 
        ...
        json.dump(metadata, open(os.path.join(folder, 'metadata' + '.json'), 'w'), indent=4)
        print(f"Data saved in {folder}")
        with open(os.path.join(folder,"scalers_features.pkl"), "wb") as f:
            pickle.dump(process.scalers_features, f)
        with open(os.path.join(folder,"scalers_labels.pkl"), "wb") as f:
            pickle.dump(process.scalers_labels, f)
